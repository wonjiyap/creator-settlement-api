...
- 조회 쿼리는 **QueryDSL** 사용. 엑셀(xlsx) 내보내기는 **Apache POI(poi-ooxml)**.
- CSV/엑셀 내보내기는 **컨트롤러(표현 계층)에서 변환** — 서비스의 `list`를 재사용하고 직렬화만 담당(`SettlementController`의 공용 컬럼 정의를 CSV/엑셀이 공유).
...
- 선택(가산점): 정산 상태 관리(PENDING→CONFIRMED→PAID) ✅ 구현, 동일 기간 중복 정산 방지 ✅ 구현, 수수료율 변경 이력(과거 정산은 당시 수수료율 적용) ✅ 구현, CSV/엑셀 다운로드 ✅ 구현 — 선택 과제 전부 완료.
